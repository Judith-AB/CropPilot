...
import { playActionSound } from '@/services/audio'; // <--- NEW IMPORT
...
            }));
            ...
            // 3. TRIGGER SOUND BASED ON OUTCOME (NEW LOGIC)
            if (apiResponse.healthChange > 0) {
                playActionSound('success'); // Play success sound for positive health change
            } else if (apiResponse.healthChange < 0) {
                playActionSound('penalty'); // Play penalty sound for negative health change
            } else {
                playActionSound('neutral'); // Play neutral sound otherwise
            }
            
        } catch (error) {
